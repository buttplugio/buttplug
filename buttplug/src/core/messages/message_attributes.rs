...
impl DeviceMessageAttributes {
  ...
  fn check_feature_count_validity(
    &self,
    message_type: &ButtplugDeviceMessageType,
  ) -> Result<(), String> {
  ...
  fn check_step_count(&self, message_type: &ButtplugDeviceMessageType) -> Result<(), String> {
    ...
    } else if self.step_count.as_ref().expect("Checked").len()
      != *self
        .feature_count
        .as_ref()
        .expect("Already checked in feature count check.") as usize
    {
      Err(format!(
        "Step count array length must match feature count for {}.",
        message_type
      ))
    } else {
      Ok(())
  ...
  fn check_step_range(&self, message_type: &ButtplugDeviceMessageType) -> Result<(), String> {
    if let Some(step_range) = &self.step_range {
      ...
      {
        Err(format!(
          "Step range array length must match feature count for {}.",
          message_type
        ))
      } else if step_range.iter().any(|range| {
        info!("{:?}", range);
        range.1 <= range.0
      }) {
        Err(format!(
          "Step range array values must have an increasing range for {}.",
          message_type
        ))
      } else if step_range.iter().enumerate().any(|(index, range)| {
        range.1
          > self
            .step_count
            .as_ref()
            .expect("Already checked in step count check")[index]
      }) {
        Err(format!(
  ...
  fn check_feature_order(&self, message_type: &ButtplugDeviceMessageType) -> Result<(), String> {
    if let Some(feature_order) = &self.feature_order {
      ...
      {
        Err(format!(
          "Feature order must have the same number of elements as feature count for {}.",
          message_type
        ))
      } else {
        Ok(())
  ...
  pub fn check(&self, message_type: &ButtplugDeviceMessageType) -> Result<(), ButtplugDeviceError> {
    match message_type {
      ...
      | ButtplugDeviceMessageType::LinearCmd => self
        .check_feature_count_validity(message_type)
      ...
      | ButtplugDeviceMessageType::RawUnsubscribeCmd => self
        .endpoints
        .is_some()
        .then(|| ())
        .ok_or(format!("Endpoints vector must exist for {}.", message_type)),
      _ => Ok(()),
    }
  ...
  pub fn merge(&self, other: &DeviceMessageAttributes) -> DeviceMessageAttributes {
    DeviceMessageAttributes {
      ...
      feature_descriptors: other
        .feature_descriptors
        .clone()
        .or_else(|| self.feature_descriptors.clone()),
      actuator_type: other
        .actuator_type
        .clone()
        .or_else(|| self.actuator_type.clone()),
      sensor_type: other
        .sensor_type
        .clone()
        .or_else(|| self.sensor_type.clone()),
      ...
      max_duration: other
        .max_duration
        .clone()
        .or_else(|| self.max_duration.clone()),
      step_range: other.step_range.clone().or_else(|| self.step_range.clone()),
      feature_order: other
        .feature_order
        .clone()
        .or_else(|| self.feature_order.clone()),
    }
  }
}
