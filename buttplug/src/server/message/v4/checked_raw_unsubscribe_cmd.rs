...
pub struct CheckedRawUnsubscribeCmdV2 {
...
impl CheckedRawUnsubscribeCmdV2 {
...
impl ButtplugMessageValidator for CheckedRawUnsubscribeCmdV2 {
  fn is_valid(&self) -> Result<(), ButtplugMessageError> {
    self.is_not_system_id(self.id)
  }
}

impl TryFromDeviceAttributes<RawUnsubscribeCmdV2> for CheckedRawUnsubscribeCmdV2 {
  fn try_from_device_attributes(
    ...
    if let Some(raw_feature) = features.features().iter().find(|x| x.raw().is_some()) {
      if raw_feature.raw().as_ref().unwrap().endpoints().contains(&msg.endpoint()) {
        Ok(CheckedRawUnsubscribeCmdV2 { id: msg.id(), device_index: msg.device_index(), endpoint: msg.endpoint() })
      } else {
