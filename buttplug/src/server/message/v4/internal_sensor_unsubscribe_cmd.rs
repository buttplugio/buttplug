...
#[getset(get_copy = "pub")]
pub struct InternalSensorUnsubscribeCmdV4 {
  id: u32,
  device_index: u32,
  feature_index: u32,
...
impl InternalSensorUnsubscribeCmdV4 {
  pub fn new(
    device_index: u32,
    feature_index: u32,
    sensor_type: SensorType,
...
impl ButtplugMessageValidator for InternalSensorUnsubscribeCmdV4 {
  fn is_valid(&self) -> Result<(), ButtplugMessageError> {
    self.is_not_system_id(self.id)
  }
}

impl TryFromDeviceAttributes<SensorUnsubscribeCmdV4> for InternalSensorUnsubscribeCmdV4 {
    ...
    if let Some(feature) = features.features().get(*msg.feature_index() as usize) {
      if let Some(sensor) = feature.sensor() {
        if sensor.messages().contains(&ButtplugSensorFeatureMessageType::SensorSubscribeCmd) {
          Ok(InternalSensorUnsubscribeCmdV4::new(msg.device_index(), *msg.feature_index(), *msg.sensor_type(), *feature.id()))
        } else {
