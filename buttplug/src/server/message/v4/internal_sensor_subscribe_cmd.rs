...
#[getset(get_copy = "pub")]
pub struct InternalSensorSubscribeCmdV4 {
  id: u32,
  device_index: u32,
  feature_index: u32,
...
impl InternalSensorSubscribeCmdV4 {
  pub fn new(
    device_index: u32,
    feature_index: u32,
    sensor_type: SensorType,
...
impl ButtplugMessageValidator for InternalSensorSubscribeCmdV4 {
  fn is_valid(&self) -> Result<(), ButtplugMessageError> {
    self.is_not_system_id(self.id)
  }
}

impl TryFromDeviceAttributes<SensorSubscribeCmdV4> for InternalSensorSubscribeCmdV4 {
    ...
    if let Some(feature) = features.features().get(*msg.feature_index() as usize) {
      if let Some(sensor) = feature.sensor() {
        if sensor.messages().contains(&ButtplugSensorFeatureMessageType::SensorSubscribeCmd) {
          Ok(InternalSensorSubscribeCmdV4::new(msg.device_index(), *msg.feature_index(), *msg.sensor_type(), *feature.id()))
        } else {
