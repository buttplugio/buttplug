...
impl ButtplugJSONServerWrapper {
  ...
  #[allow(dead_code)]
  pub fn new(name: &str, max_ping_time: u128) -> (Self, Receiver<String>) {
    ...
    if let Some(version) = self.message_version {
      ...
      task::spawn(async move {
        while let Some(event) = recv_server.next().await {
          let converted_event =
            ButtplugJSONServerWrapper::convert_outgoing_associated(version, event);
  ...
  fn convert_outgoing_associated(
    version: ButtplugMessageSpecVersion,
    msg: ButtplugOutMessage,
  ) -> String {
    match version {
      ButtplugMessageSpecVersion::Version0 => match ButtplugSpecV0OutMessage::try_from(msg) {
        Ok(msgv0) => msgv0.as_protocol_json(),
        Err(err) => {
          ButtplugSpecV0OutMessage::Error(ButtplugError::ButtplugMessageError(err).into())
            .as_protocol_json()
        }
      },
      ButtplugMessageSpecVersion::Version1 => match ButtplugSpecV1OutMessage::try_from(msg) {
        Ok(msgv1) => msgv1.as_protocol_json(),
        Err(err) => {
          ButtplugSpecV1OutMessage::Error(ButtplugError::ButtplugMessageError(err).into())
            .as_protocol_json()
        }
      },
      ButtplugMessageSpecVersion::Version2 => match ButtplugSpecV2OutMessage::try_from(msg) {
        Ok(msgv2) => msgv2.as_protocol_json(),
        Err(err) => {
          ButtplugSpecV2OutMessage::Error(ButtplugError::ButtplugMessageError(err).into())
            .as_protocol_json()
        }
      },
    }
  ...
  fn convert_outgoing(&self, msg: ButtplugOutMessage) -> String {
    if let Some(version) = self.message_version {
      ...
      if let ButtplugOutMessage::Error(_) = &msg {
        ButtplugJSONServerWrapper::convert_outgoing_associated(
          ButtplugMessageSpecVersion::Version2,
          msg.clone(),
        )
      } else {
