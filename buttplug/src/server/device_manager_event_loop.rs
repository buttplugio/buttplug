...
/*
pub fn wait_for_manager_events(
  device_config_manager: Arc<DeviceConfigurationManager>,
  server_sender: broadcast::Sender<ButtplugServerMessage>,
  device_map: Arc<DashMap<u32, ButtplugDevice>>,
  ping_notifier: Option<Arc<PingTimer>>,
  mut device_comm_receiver: mpsc::Receiver<DeviceCommunicationEvent>
) -> impl Future<Output = ()>
{
  let main_device_index = Arc::new(AtomicU32::new(0));
  let device_index_map: Arc<DashMap<String, u32>> = Arc::new(DashMap::new());
  let (device_event_sender, mut device_event_receiver) = mpsc::channel::<(u32, ButtplugDeviceEvent)>(256);
  ...
  let ping_receiver = server_sender.subscribe();
  let event_loop = async move {
    loop {
      let ping_fut = async {
        if let Ok(recv) = &ping_receiver {
