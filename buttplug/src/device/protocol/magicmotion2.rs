...
pub struct MagicMotion2ProtocolCreator {
    config: DeviceProtocolConfiguration,
}

impl MagicMotion2ProtocolCreator {
...
#[async_trait]
impl ButtplugProtocolCreator for MagicMotion2ProtocolCreator {
    async fn try_create_protocol(
        ...
        Ok(Box::new(MagicMotion2Protocol::new(name, attrs)))
    }
}

#[derive(Clone)]
pub struct MagicMotion2Protocol {
    name: String,
...
#[async_trait]
impl ButtplugProtocol for MagicMotion2Protocol {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    async fn parse_message(
        ...
        match message {
            ...
            _ => Err(ButtplugError::ButtplugDeviceError(
                ButtplugDeviceError::new("MagicMotion2Protocol does not accept this message type."),
            )),
        }
    }
}
...
impl MagicMotion2Protocol {
    ...
    async fn handle_vibrate_cmd(
        ...
        let msg = DeviceWriteCmd::new(Endpoint::Tx, vec![0x10, 0xff, 0x04, 0x0a, 0x32, 0x0a, 0x00, 0x04, 0x08, self.vibrations[0], 0x64, 0x00, 0x04, 0x08, if self.vibrations.len() >= 2 {self.vibrations[1]} else {0x00}, 0x64, 0x01], false);
