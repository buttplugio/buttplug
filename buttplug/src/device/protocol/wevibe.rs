...
pub struct WeVibeProtocolCreator {
    config: DeviceProtocolConfiguration,
}

impl WeVibeProtocolCreator {
...
#[async_trait]
impl ButtplugProtocolCreator for WeVibeProtocolCreator {
    async fn try_create_protocol(
        ...
        Ok(Box::new(WeVibeProtocol::new(name, attrs)))
    }
}

#[derive(Clone)]
pub struct WeVibeProtocol {
    name: String,
...
#[async_trait]
impl ButtplugProtocol for WeVibeProtocol {
    fn name(&self) -> &str {
        &self.name
    }
...
impl WeVibeProtocol {
    ...
    async fn handle_vibrate_cmd(
        ...
        data[3] = self.vibrations[self.vibrations.len() - 1];
        data[3] |= self.vibrations[0] << 4;
