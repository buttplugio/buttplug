...
use buttplug_server_device_config::{load_protocol_configs, UserDeviceCustomization, DeviceDefinition, UserDeviceIdentifier, ServerDeviceFeature, ServerDeviceFeatureOutput};
use buttplug_server::{
    device::{
...
async fn test_client_repeated_deviceadded_message() {
  use buttplug_core::message::OkV0;
  ...
  async_manager::spawn(async move {
    use buttplug_core::message::{ButtplugClientMessageV4, DeviceAddedV4};
...
async fn test_client_repeated_deviceremoved_message() {
  use buttplug_core::message::{DeviceRemovedV0, OkV0};
  use buttplug_server::message::{ButtplugClientMessageVariant, ButtplugServerMessageVariant};
