...
pub struct ButtplugInProcessClientConnector {
  ...
  server_outbound_sender: Sender<ButtplugServerMessageV0>,
...
impl ButtplugConnector<ButtplugClientMessageV0, ButtplugServerMessageV0>
  for ButtplugInProcessClientConnector
{
  fn connect(
    &mut self,
    message_sender: Sender<ButtplugServerMessageV0>,
      ...
      async move {
        buttplug_core::spawn!("InProcessClientConnector event sender loop", async move {
          ...
          while let Some(event) = server_recv.next().await {
            ...
            if let ButtplugServerMessageVariant::V0(msg) = event {
  ...
  fn send(&self, msg: ButtplugClientMessageV0) -> ButtplugConnectorResultFuture {
    if !self.connected.load(Ordering::Relaxed) {
    ...
    async move {
      let output = match output_fut.await {
        Ok(m) => {
          if let ButtplugServerMessageVariant::V0(msg) = m {
            msg
          } else {
            ButtplugServerMessageV0::Error(
        ...
        Err(e) => {
          if let ButtplugServerMessageVariant::V0(msg) = e {
            msg
          } else {
            ButtplugServerMessageV0::Error(
              ButtplugError::from(ButtplugMessageError::MessageConversionError(
