...
use crate::util::{
  ButtplugTestServer, TestDeviceChannelHost, device_test::connector::build_channel_connector_v0,
};
...
pub async fn run_json_test_case(test_case: &DeviceTestCase) {
  ...
  let (client_connector, server_connector) = build_channel_connector_v0(&notify);
...
pub async fn run_test_case(
  ...
  // Parse send message into client calls, receives into response checks
  for command in filter_commands(&test_case.device_commands, 0) {
    match command {
      TestCommand::Messages {
        device_index,
        messages,
