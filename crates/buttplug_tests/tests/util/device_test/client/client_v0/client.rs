...
// The v0 client implementation for protocol version 0
...
pub(super) type ButtplugServerMessageResult = ButtplugClientResult<ButtplugServerMessageV0>;
...
pub struct ButtplugClientMessageFuturePair {
  pub msg: ButtplugClientMessageV0,
  pub sender: Option<ButtplugServerMessageSender>,
}

impl ButtplugClientMessageFuturePair {
  pub fn new(msg: ButtplugClientMessageV0, sender: ButtplugServerMessageSender) -> Self {
    Self {
      msg,
      sender: Some(sender),
    }
...
impl ButtplugClient {
  ...
  async fn run_handshake(&self) -> ButtplugClientResult {
    ...
    let msg = self
      .send_message_ignore_connect_status(
        RequestServerInfoV1::new(&self.client_name, ButtplugMessageSpecVersion::Version0).into(),
      )
      .await?;

    debug!("Got ServerInfo return.");
    if let ButtplugServerMessageV0::ServerInfo(server_info) = msg {
      ...
      *self.server_name.lock().await = Some(server_info.server_name().clone());
      ...
      let msg = self
        ...
        .await?;
      if let ButtplugServerMessageV0::DeviceList(m) = msg {
        self
  ...
  fn send_message(&self, msg: ButtplugClientMessageV0) -> ButtplugServerMessageResultFuture {
    if !self.connected() {
