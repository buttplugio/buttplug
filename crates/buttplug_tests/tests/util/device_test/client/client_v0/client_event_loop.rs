...
use tokio::sync::{broadcast, mpsc, mpsc::Sender as MpscSender};

/// Type alias for disconnect sender
pub(super) type ButtplugConnectorStateSender =
...
where
  ConnectorType: ButtplugConnector<ButtplugClientMessageV0, ButtplugServerMessageV0> + 'static,
{
  ...
  from_connector_receiver: mpsc::Receiver<ButtplugServerMessageV0>,
  ...
  from_client_sender: MpscSender<ButtplugClientRequest>,
  ...
  ConnectorType: ButtplugConnector<ButtplugClientMessageV0, ButtplugServerMessageV0> + 'static,
{
  ...
  /// Creates a [ButtplugClientDevice] from V0 device fields.
  ///
  /// Given device information from a [DeviceAdded] or [DeviceList] message,
  /// creates a ButtplugClientDevice and adds it the internal device map, then
  /// returns the instance.
  fn create_client_device(
    &mut self,
    device_index: u32,
    device_name: &str,
    ...
    match self.device_map.get(&device_index) {
      ...
      None => {
        ...
        let device = Arc::new(ButtplugClientDevice::new_from_device_fields(
          device_index,
          device_name,
          device_messages,
  ...
  async fn parse_connector_message(&mut self, msg: ButtplugServerMessageV0) {
    ...
    info!("{:?}", msg);
    match msg {
      ButtplugServerMessageV0::DeviceAdded(dev) => {
