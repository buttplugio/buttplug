...
use crate::util::{
  ButtplugTestServer, TestDeviceChannelHost, device_test::connector::build_channel_connector_v3,
};
...
pub async fn run_json_test_case(test_case: &DeviceTestCase) {
  ...
  let (client_connector, server_connector) = build_channel_connector_v3(&notify);
