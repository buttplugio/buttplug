...
// The last version of the v1 client, adapted from the v2 client
...
use buttplug_server::message::{
  ButtplugClientMessageV1, ButtplugServerMessageV1, RequestServerInfoV1, StopAllDevicesV0,
};
use dashmap::DashMap;
...
pub(super) type ButtplugServerMessageResult = ButtplugClientResult<ButtplugServerMessageV1>;
pub(super) type ButtplugServerMessageResultFuture =
  ButtplugClientResultFuture<ButtplugServerMessageV1>;
...
pub struct ButtplugClientMessageFuturePair {
  pub msg: ButtplugClientMessageV1,
  pub sender: Option<ButtplugServerMessageSender>,
}

impl ButtplugClientMessageFuturePair {
  pub fn new(msg: ButtplugClientMessageV1, sender: ButtplugServerMessageSender) -> Self {
    Self {
      msg,
      sender: Some(sender),
    }
...
impl ButtplugClient {
  ...
  where
    ConnectorType: ButtplugConnector<ButtplugClientMessageV1, ButtplugServerMessageV1> + 'static,
  {
    if self.connected() {
  ...
  async fn run_handshake(&self) -> ButtplugClientResult {
    ...
    let msg = self
      .send_message_ignore_connect_status(
        RequestServerInfoV1::new(&self.client_name, ButtplugMessageSpecVersion::Version1).into(),
      )
      .await?;

    debug!("Got ServerInfo return.");
    if let ButtplugServerMessageV1::ServerInfo(server_info) = msg {
      ...
      *self.server_name.lock().await = Some(server_info.server_name().clone());
      ...
      let msg = self
        ...
        .await?;
      if let ButtplugServerMessageV1::DeviceList(m) = msg {
        self
  ...
  fn send_message(&self, msg: ButtplugClientMessageV1) -> ButtplugServerMessageResultFuture {
    if !self.connected() {
  ...
  fn send_message_expect_ok(&self, msg: ButtplugClientMessageV1) -> ButtplugClientResultFuture {
