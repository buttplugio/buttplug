...
pub enum ButtplugClientDeviceEvent {
  ...
  Message(ButtplugServerMessageV1),
}
...
pub struct ButtplugClientDevice {
  ...
  /// messages.
  #[getset(get = "pub")]
  message_attributes: ClientDeviceMessageAttributesV1,
...
impl ButtplugClientDevice {
  ...
  pub(super) fn new(
    name: &str,
    index: u32,
    allowed_messages: ClientDeviceMessageAttributesV1,
  ...
  fn send_message(
    ...
    Box::pin(
      async move {
        ...
        if let ButtplugServerMessageV1::Error(_err) = msg {
  ...
  fn send_message_expect_ok(&self, msg: ButtplugClientMessageV1) -> ButtplugClientResultFuture {
    ...
    Box::pin(async move {
      match send_fut.await? {
        ButtplugServerMessageV1::Ok(_) => Ok(()),
        ButtplugServerMessageV1::Error(_err) => Err(ButtplugError::from(_err).into()),
        msg => Err(
    ...
    let vibrator_count: u32 = if let Some(features) = self.message_attributes.vibrate_cmd() {
      ...
      return self.create_boxed_future_client_error(
        ButtplugDeviceError::MessageNotSupported(
          ButtplugDeviceMessageNameV1::VibrateCmd.to_string(),
        )
        .into(),
      );
    };
  ...
  pub fn linear(&self, linear_cmd: LinearCommand) -> ButtplugClientResultFuture {
    let linear_count: u32 = if let Some(features) = self.message_attributes.linear_cmd() {
      ...
      return self.create_boxed_future_client_error(
        ButtplugDeviceError::MessageNotSupported(
          ButtplugDeviceMessageNameV1::LinearCmd.to_string(),
        )
        .into(),
      );
    };
  ...
  pub fn rotate(&self, rotate_cmd: RotateCommand) -> ButtplugClientResultFuture {
    let rotate_count: u32 = if let Some(features) = self.message_attributes.rotate_cmd() {
      ...
      return self.create_boxed_future_client_error(
        ButtplugDeviceError::MessageNotSupported(
          ButtplugDeviceMessageNameV1::RotateCmd.to_string(),
        )
        .into(),
      );
    };
