...
use crate::util::{
  ButtplugTestServer, TestDeviceChannelHost, device_test::connector::build_channel_connector_v1,
};
...
async fn run_test_client_command(command: &TestClientCommand, device: &Arc<ButtplugClientDevice>) {
  ...
  match command {
    ...
    Battery { .. } => {
      panic!("Battery not supported in v1");
    }
    _ => {
      panic!(
...
pub async fn run_json_test_case(test_case: &DeviceTestCase) {
  ...
  let (client_connector, server_connector) = build_channel_connector_v1(&notify);
...
pub async fn run_test_case(
  ...
  if let Some(device_init) = &test_case.device_init {
    ...
    for command in filter_commands(device_init, 1) {
      match command {
  ...
  // Parse send message into client calls, receives into response checks
  for command in filter_commands(&test_case.device_commands, 1) {
    match command {
      TestCommand::Messages {
        device_index,
        messages,
