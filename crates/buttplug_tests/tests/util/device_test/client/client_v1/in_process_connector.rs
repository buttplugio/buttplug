...
use buttplug_server::{
  ...
  message::{ButtplugClientMessageV1, ButtplugServerMessageV1, ButtplugServerMessageVariant},
};
use futures::{
  StreamExt,
...
pub struct ButtplugInProcessClientConnector {
  ...
  server_outbound_sender: Sender<ButtplugServerMessageV1>,
...
impl ButtplugConnector<ButtplugClientMessageV1, ButtplugServerMessageV1>
  for ButtplugInProcessClientConnector
{
  fn connect(
    &mut self,
    message_sender: Sender<ButtplugServerMessageV1>,
      ...
      async move {
        buttplug_core::spawn!("InProcessClientConnector event sender loop", async move {
          ...
          while let Some(event) = server_recv.next().await {
            ...
            if let ButtplugServerMessageVariant::V1(msg) = event {
  ...
  fn send(&self, msg: ButtplugClientMessageV1) -> ButtplugConnectorResultFuture {
    if !self.connected.load(Ordering::Relaxed) {
    ...
    async move {
      let output = match output_fut.await {
        Ok(m) => {
          if let ButtplugServerMessageVariant::V1(msg) = m {
            msg
          } else {
            ButtplugServerMessageV1::Error(
        ...
        Err(e) => {
          if let ButtplugServerMessageVariant::V1(msg) = e {
            msg
          } else {
            ButtplugServerMessageV1::Error(
              ButtplugError::from(ButtplugMessageError::MessageConversionError(
