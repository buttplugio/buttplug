...
pub mod setup {
    use crate::device::protocol::{ProtocolIdentifierFactory, ProtocolIdentifier};

    #[derive(Default)]
    pub struct TheHandyV3IdentifierFactory {}

    impl ProtocolIdentifierFactory for TheHandyV3IdentifierFactory {
    fn identifier(&self) -> &str { "thehandy-v3" }
...
impl ProtocolIdentifier for TheHandyV3Identifier {
    async fn identify(
        ...
        Ok((UserDeviceIdentifier::new(hardware.address(), "thehandy-v3", &Some(name.to_owned())), Box::new(TheHandyV3Initializer::default())))
    }
}
...
impl ProtocolInitializer for TheHandyV3Initializer {
    async fn initialize(
        ...
        hardware
            .subscribe(&HardwareSubscribeCmd::new(THEHANDY_V3_SUBSCRIBE_UUID, Endpoint::Rx))
            .await?;
...
impl ProtocolHandler for TheHandyV3 {
    ...
    fn handle_output_vibrate_cmd(&self, _feature_index: u32, feature_id: Uuid, speed: u32) -> Result<Vec<HardwareCommand>, ButtplugDeviceError> {
        ...
        let mut buf = vec![];
        payload.encode(&mut buf).expect("Infallible encode.");Ok(vec![
