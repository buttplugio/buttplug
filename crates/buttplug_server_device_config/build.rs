...
fn main() {
  ...
  for protocol_file in std::fs::read_dir(PROTOCOL_DIR).unwrap() {
    ...
    output.protocols.insert(
      f.file_name()
        ...
        .unwrap()
        .to_owned(),
      serde_yaml::from_str(&std::fs::read_to_string(f.path()).unwrap()).unwrap(),
    );
  }
  ...
  std::fs::write(
    VERSION_FILE,
    serde_yaml::to_string(&version).unwrap().as_bytes(),
  )
  .unwrap();
