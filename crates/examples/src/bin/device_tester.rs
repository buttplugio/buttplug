...
use buttplug_core::message::{DeviceFeature, DeviceFeatureOutput, OutputCommand, OutputRotateWithDirection, OutputType, OutputValue};
            ...
            for (_, feature) in dev.device_features() {
                for outputs in feature.feature().output() {
                    for otype in outputs.keys() {
                        ...
                        match otype {
                            OutputType::Vibrate | OutputType::Rotate | OutputType::Constrict | OutputType::Oscillate | OutputType::Heater | OutputType::Spray | OutputType::Led | OutputType::Position  => {
                            ...
                            OutputType::RotateWithDirection => {
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection(output.step_count() / 4, true)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 25% clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection(output.step_count() / 4, false)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 25% anti-clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection(output.step_count() / 2, true)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 50% clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection(output.step_count() / 2, false)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 50% anti-clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection((output.step_count() / 4) * 3, true)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 75% clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection((output.step_count() / 4) * 3, false)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 75% anti-clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection(output.step_count(), true)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 100% clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection(output.step_count(), false)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 100% anti-clockwise",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
                                    ...
                                    otype);
                                sleep(Duration::from_secs(1)).await;
                                feature.send_command(&ClientDeviceOutputCommand::RotateWithDirection(0, true)).await;
                                println!(
                                    "{} ({}) Testing feature {}: {} ({}), output {:?} - 0%",
                                    dev.name(),
                                    dev.index(),
                                    feature.feature().feature_index(),
                                    feature.feature().feature_type(),
