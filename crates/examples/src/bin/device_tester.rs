...
async fn set_level_and_wait(dev: &Arc<ButtplugClientDevice>, feature: &ClientDeviceFeature, output_type: &OutputType, level: f64) {
...
async fn set_rotate_with_direction_and_wait(dev: &Arc<ButtplugClientDevice>, feature: &ClientDeviceFeature, output_type: &OutputType, level: f64, clockwise: bool) {
    feature.send_command(&ClientDeviceOutputCommand::RotateWithDirectionFloat(level, clockwise)).await.unwrap();
    println!(
        "{} ({}) Testing feature {}: {} ({}), output {:?} - {}% {}",
        dev.name(),
        dev.index(),
...
async fn device_tester() {
    ...
    let exercise_device = |dev: Arc<ButtplugClientDevice>| {
        async move {
            ...
            // Start all "scalar" features at 100% for 5 seconds simultaneously
            let mut cmds = vec![];
            ...
            // Stop all "scalar" features simultaneously
            let mut cmds = vec![];
            dev.device_features().iter().for_each(|(_, feature)| {
                let outs = feature.feature().output().clone().unwrap_or_default();
                if let Some(out) = outs.get(&OutputType::Vibrate) {
                    ...
                    for otype in outputs.keys() {
                        ...
                        match otype {
                            OutputType::Vibrate | OutputType::Rotate | OutputType::Constrict | OutputType::Oscillate | OutputType::Heater | OutputType::Spray | OutputType::Led | OutputType::Position  => {
                                // Do "scalar" features at increasing levels
                            ...
                            OutputType::RotateWithDirection => {
                                // Rotate at increasing speed alternating direction
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.25, true).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.25, false).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.5, true).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.5, false).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.75, true).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.75, false).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 1.0, true).await;
                                ...
                                // Rotate at increasing speed one direction at a time
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.25, true).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.5, true).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 0.75, true).await;
                                set_rotate_with_direction_and_wait(&dev, feature, otype, 1.0, true).await;
